...
- `resukisu_susfs`：已验证的 ReSukiSU + SUSFS 层。ReSukiSU 自带 SUSFS Kconfig/compat 代码，因此这个路径会跳过 SukiSU 专用 SUSFS adapter。
...
已验证的 ReSukiSU + SUSFS 基线：

- Run: `28494030221`
- 模式：`root_solution=resukisu_susfs`，`variant=gki`，`enable_kpm=false`
- 结果：成功
- 证据：ReSukiSU 集成和 SUSFS patch 均完成；`build-asteroids.log` 记录 `ReSukiSU: using SuSFS Inline hook`、`SUSFS_VERSION: v2.2.0`，并以 `Build completed successfully` 结束。
- 配置证据：`CONFIG_KSU=y`、`CONFIG_KSU_SUSFS=y`，并且 ReSukiSU 没有 `CONFIG_KPM` 配置项。
